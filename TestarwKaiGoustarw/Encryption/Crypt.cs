...
namespace TestarwKaiGoustarw.Encryption
{
    public static class Crypt
    {
        ...
        {
            CryptFile(password, currentPath, destinationPath, true);
        }
        ...
        private static void CryptFile(string password,
            string inFile, string outFile, bool encrypt)
        {
        ...
        private static void CryptStream(string password,
            Stream inStream, Stream outStream, bool encrypt)
        {
            ...
            int blockSizeBits = aesProvider.BlockSize;
            ...
            ICryptoTransform cryptoTransform = encrypt ? aesProvider.CreateEncryptor(key, iv) : aesProvider.CreateDecryptor(key, iv);
            ...
            try
            {
                        ...
                        CryptoStreamMode.Write))
                {
                    ...
                    {
                        // Read some bytes.
                        int bytesRead = inStream.Read(buffer, 0, BLOCK_SIZE);
                        if (bytesRead == 0)
                        {
                            break;
                        }
